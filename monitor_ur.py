...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=
        "skip persistence updates while still performing scraping and diffing",
    )
    parser.add_argument(
        "--target-url",
        default=os.getenv(
...
def main(argv: list[str] | None = None) -> int:
    ...
    new_rooms = [
        room for diff in summary.room_diffs.values() for room in diff.added
    ]
    if new_rooms:
    ...
    if summary.availability_changes:
        ...
        for change in summary.availability_changes.values():
            previous = (str(change.previous_count)
                        if change.previous_count is not None else "N/A")
            logger.info(
                "%s | 対象空室数: %s -> %s | %s",
    ...
    if not args.dry_run:
        try:
            timestamp = (summary.executed_at.replace(":", "-").replace(
                ".", "-").replace("T", "_"))
